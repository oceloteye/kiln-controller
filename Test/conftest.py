...
async def config_server():
    """Start a simple /config websocket server on a random port and set KILN_SERVER.
    Yields the websocket URL; server is closed after tests in the module finish."""
    ...
    async def config_server():
        ...
        def http_server():
            ...
            # serve the repo's public/ directory
            repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
            public_dir = os.path.join(repo_root, 'public')

            class _Handler(SimpleHTTPRequestHandler):
                def translate_path(self, path):
                    # normalize and map /picoreflow/... to public/...
                    if path.startswith('/picoreflow/'):
                        path = path[len('/picoreflow'):]
                ...
                def do_GET(self):
                    # respond 200 to reserved endpoints so the page JS doesn't get 404
                    if self.path in ('/status', '/control', '/config', '/storage'):
